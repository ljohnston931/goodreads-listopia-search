...
class ListService {
    ...
    async getListsForBookFromGoodreads(bookId) {
        const firstPageResp = await http.medium.get(`https://www.goodreads.com/list/book/${bookId}`)
        ...
        const otherPagesPromises = otherPages.map(async pageNumber =>
            http.medium.get(`https://www.goodreads.com/list/book/${bookId}?page=${pageNumber}`)
        )
    ...
    async getAuthorLists(authorIds) {
        return await Promise.all(authorIds.map(async authorId => this.getListsForAuthor(authorId)))
    }
    ...
    async getListsForAuthor(authorId) {
        ...
        const bookIdsInDatabase = [...new Set(bookListsFromDatabase.map(list => list.book_id))]
        const bookIdsNotInDatabase = bookIds.filter(bookId => !bookIdsInDatabase.includes(bookId))
        console.log('ids in database', bookIdsInDatabase)
        console.log('ids not in database', bookIdsNotInDatabase)
        console.log('getting books for authors')
        const bookListsFromGoodreads = await Promise.all(
            bookIdsNotInDatabase.map(bookId => this.getListsForBookFromGoodreads(bookId))
        )
